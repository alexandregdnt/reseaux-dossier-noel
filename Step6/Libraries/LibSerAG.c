...
void saisieFactureAG(int NumeroFacturation, struct FactureAG *UneFacture) {
    ...
    printf("Saisie DateFacturation: ");
    fgets(Tampon, sizeof Tampon, stdin);
    UneFacture->DateFacturation = atoi(Tampon);
    // si vérif -> do while (UneFacture->DateFacturation < 010100 || UneFacture->DateFacturation > 123123);
...
int rechercheFactureAG(char *NomFichier, char *NomClient, int DateFacture, struct FactureAG *UneFacture) {
    FILE *fp;
    fp = fopen(NomFichier, "r+");
    ...
    if (fp != NULL) {
        ...
        while (!feof(fp) && strcmp(NomClient, UneFacture->NomClient) != 0 && DateFacture != UneFacture->DateFacturation) {
            fread(UneFacture, sizeof(*UneFacture), 1, fp);
        }
    } else {
        return -1;
    ...
    fclose(fp);
    return (strcmp(NomClient, UneFacture->NomClient) == 0 && DateFacture == UneFacture->DateFacturation);
}
...
void afficheFactureAG(struct FactureAG *UneFacture) {
    ...
    if (strlen(Tampon) < 6) {
        char Tampon2[10];
        strcpy(Tampon2, Tampon);
        strcpy(Tampon, "");
        for (int i=0; strlen(Tampon2)+i < 6; i++) {
            strcat(Tampon, "0");
        }
        strcat(Tampon, Tampon2);
    }
    MonPrintf(Tampon, 8, strlen(Tampon));
